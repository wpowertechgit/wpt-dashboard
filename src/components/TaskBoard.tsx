        ...
        <Stack gap={2}>
          ...
          {(canManage || isOwner) && (
            <AppSelect
              ...
              options={[
                { value: 'TODO', label: tk.statusTodo },
                { value: 'IN_PROGRESS', label: tk.statusInProgress },
                { value: 'DONE', label: tk.statusDone },
              ]}
            />
          )}
      ...
      <Stack component="form" onSubmit={submit} gap={1.5}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, 1fr)' }, gap: 1.5 }}>
          ...
          <AppSelect label={tk.priority} value={form.priority} onChange={e => setF('priority', e.target.value)}
            options={[
              { value: 'LOW', label: tk.priorityLow }, { value: 'NORMAL', label: tk.priorityNormal },
              { value: 'HIGH', label: tk.priorityHigh }, { value: 'URGENT', label: tk.priorityUrgent },
            ]} />
  ...
  function taskRow(task: Task) {
    ...
    return (
      <TableRow key={task.id} sx={{ cursor: 'pointer', '&:hover': { bgcolor: 'var(--color-surface-2)' } }} onClick={() => setDetailTask(task)}>
        <TableCell sx={{ fontWeight: 500, fontSize: 13, maxWidth: 280 }}>
          <Typography sx={{ fontSize: 13, fontWeight: 500, color: task.status === 'DONE' ? 'var(--color-ink-tertiary)' : 'var(--color-ink)', textDecoration: task.status === 'DONE' ? 'line-through' : 'none' }}>{task.title}</Typography>
          {task.description && <Typography sx={{ fontSize: 11, color: 'var(--color-ink-tertiary)', mt: 0.25, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 260 }}>{task.description}</Typography>}
        </TableCell>
        ...
        <TableCell sx={{ fontSize: 12, color: task.due_date && new Date(task.due_date) < new Date() && task.status !== 'DONE' ? '#f87171' : 'var(--color-ink-tertiary)' }}>{task.due_date ?? '—'}</TableCell>
  ...
  return (
    <UiStack gap={4}>
      <PageTitle
        eyebrow={tk.eyebrow}
        title={tk.title}
        subtitle={`${displayTasks.length} ${t.common.records}`}
        info={pageInfo(lang, 'tasks')}
        action={canCreate ? (
          <ActionButton variant={showForm ? 'outlined' : 'contained'} onClick={() => setShowForm(s => !s)}>
            {showForm ? `x ${t.common.cancel}` : tk.newBtn}
          </ActionButton>
        ) : undefined}
      />
