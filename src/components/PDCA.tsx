...
function priorityBadge(p: string) {
  ...
  return <Badge>SCÄ‚ZUT</Badge>
}
function statusBadge(s: string) {
  if (s === 'Deschis') return <Badge tone="warning">Deschis</Badge>
  if (s === 'In analiza') return <Badge tone="info">ÃŽn AnalizÄƒ</Badge>
  if (s === 'Inchis') return <Badge tone="success">ÃŽnchis</Badge>
...
export default function PDCA_View() {
  ...
  return (
    <Stack gap={4}>
      <PageTitle eyebrow={pd.eyebrow} title={pd.title} subtitle={`${pd.subtitle} â€” ${data?.length ?? 'â€¦'} Â· ${overdue.length} ${pd.overdueLabel}`} action={<ActionButton variant={showForm ? 'outlined' : 'contained'} onClick={() => setShowForm(s => !s)}>{showForm ? `âœ• ${t.common.cancel}` : pd.newBtn}</ActionButton>} />
      {error && <ErrorBanner message={error} />}

      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1.5 }}>
        {[
          { phase: 'â—† PLAN', color: '#818cf8', bg: 'rgba(94,106,210,0.08)', desc: 'Identificare problemÄƒ' },
          { phase: 'â—† DO', color: '#fbbf24', bg: 'rgba(245,158,11,0.08)', desc: 'Contramasuri' },
          { phase: 'â—† CHECK', color: '#4ade80', bg: 'rgba(39,166,68,0.08)', desc: 'Verificare rezultate' },
          { phase: 'â—† ACT', color: '#f87171', bg: 'rgba(239,68,68,0.08)', desc: 'AcÈ›iune urmÄƒ' },
        ...
        <Card sx={{ borderLeft: '3px solid var(--color-primary)' }}>
          ...
          <Stack component="form" onSubmit={submit} gap={1.5}>
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1.5 }}>
              ...
              <AppField label="SursÄƒ" value={form.sursa} onChange={e => setF('sursa', e.target.value)} placeholder="BLK-001" />
              <AppField label="Data Deschis" value={form.data_deschis} onChange={e => setF('data_deschis', e.target.value)} placeholder="12-Mai-25" />
              <AppSelect label="Proiect" value={form.proiect} onChange={e => setF('proiect', e.target.value)} options={[{ value: '', label: 'â€” SelectaÈ›i â€”' }, 'WP1000-08','WP1000-09','WP1000-10','TOATE']} />
              ...
              <AppField label="ProblemÄƒ (Plan) *" required value={form.problema} onChange={e => setF('problema', e.target.value)} sx={{ gridColumn: '1 / -1' }} />
              <AppField label="ContramasurÄƒ (Do)" value={form.contramasura} onChange={e => setF('contramasura', e.target.value)} sx={{ gridColumn: '1 / -1' }} />
      ...
      <Card sx={{ p: 0, overflow: 'hidden' }}>
        <Stack direction="row" alignItems="center" sx={{ p: '20px 24px 16px', borderBottom: '1px solid var(--color-hairline)' }}>
          ...
          {overdue.length > 0 && <Badge tone="error" sx={{ ml: 'auto' }}>{overdue.length} depÄƒÈ™ite</Badge>}
        </Stack>
        <DataTable sx={{ overflowX: 'auto' }} head={<TableRow><TableCell>{pd.colId}</TableCell><TableCell>{pd.colSursa}</TableCell><TableCell>{pd.colData}</TableCell><TableCell>{pd.colProiect}</TableCell><TableCell sx={{ minWidth: 200 }}>{pd.colProblema}</TableCell><TableCell sx={{ minWidth: 220 }}>{pd.colContramasura}</TableCell><TableCell>{pd.colResponsabil}</TableCell><TableCell>{pd.colTermen}</TableCell><TableCell>{pd.colStatus}</TableCell><TableCell>{pd.colPrioritate}</TableCell><TableCell>{pd.colZile}</TableCell><TableCell /></TableRow>}>
          {loading ? <LoadingRows cols={12} /> : (data ?? []).length === 0 ? <EmptyState label={pd.tableTitle} /> : (data ?? []).map(p => (
            <TableRow key={p.id} sx={p.zile_ramas === 'DEPASIT' ? { bgcolor: 'rgba(239,68,68,0.03)' } : undefined}>
              ...
              <TableCell>{p.zile_ramas === 'DEPASIT' ? <Badge tone="error">â›” DEPÄ‚È˜IT</Badge> : <Typography variant="body2" sx={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--color-ink-muted)' }}>{p.zile_ramas}</Typography>}</TableCell>
              <TableCell>{p.status !== 'Inchis' && <ActionButton variant="outlined" onClick={() => closeAction(p.id)} sx={{ bgcolor: 'rgba(39,166,68,0.1)', borderColor: 'rgba(39,166,68,0.2)', color: '#4ade80', fontSize: 11, px: 1, py: 0.375, whiteSpace: 'nowrap' }}>âœ… ÃŽnchide</ActionButton>}</TableCell>
            </TableRow>
          ))}
        </DataTable>
      </Card>
      ...
      <Card>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1.5 }}>
          {[
            { nivel: 'Nivel 1 â€” È˜ef EchipÄƒ', desc: 'Blocaj rezolvabil Ã®n <4 ore fÄƒrÄƒ resurse extra', color: '#4ade80' },
            { nivel: 'Nivel 2 â€” È˜ef ProducÈ›ie', desc: 'Blocaj >4 ore SAU implicÄƒ alt departament SAU aprovizionare', color: '#fbbf24' },
            { nivel: 'Nivel 3 â€” Director General', desc: 'Blocaj >1 zi SAU risc livrare proiect SAU cost suplimentar major', color: '#f87171' },
