...
export default function Proiecte() {
  ...
  async function handleSave(form: ProjectForm) {
    const isNewlyLivrat = form.status === 'LIVRAT' && editTarget?.status !== 'LIVRAT'
    if (mode === 'create') {
    ...
    setEditTarget(null)
    refetch()
    if (isNewlyLivrat) fireProjectConfetti()
  }

  function fireProjectConfetti() {
    const scalar = 2
    ...
    const defaults = { spread: 360, ticks: 60, gravity: 0, decay: 0.96, startVelocity: 20, shapes: [triangle, square, coin, tree], scalar }
    const shoot = () => {
